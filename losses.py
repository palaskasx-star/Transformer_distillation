...
def layer_mf_loss_prototypes_rand(F_s, F_t, K, normalize=False, distance='MSE', eps=1e-8, prototypes=None, projectors_net=None, KoLeoData=None, KoLeoPrototypes=None, temperature=0.1, world_size=1):
    ...
    p1 = 1 / ((1 + M_s/temperature)**((temperature+1)/2))
    p1 = p1/p1.sum(dim=2, keepdim=True)
    ...
    if distance == 'MSE':
        ...
        loss12 = - 100*temperature**2*torch.mean(torch.sum(q1 * torch.log(p2 + 1e-6), dim=2))
        loss21 = - 100*temperature**2*torch.mean(torch.sum(q2 * torch.log(p1 + 1e-6), dim=2))
