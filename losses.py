...
class DistillationLoss(nn.Module):
    ...
    def __init__(self, 
             ...
             args,
             proto_proj_module: torch.nn.Module = None,
             vitkd_projector: torch.nn.Module = None):
        super().__init__()

        self.base_criterion = base_criterion
        self.teacher_model = teacher_model
        print(args.distillation_type)
        ...
        if proto_proj_module is not None:
            self.prototypes = proto_proj_module.prototypes
            self.projectors_nets = proto_proj_module.projectors
        ...
        self.use_vitkd = args.use_vitkd
        self.vitkd_projector = vitkd_projector
        if self.use_vitkd:
            self.alpha_vitkd = getattr(args, 'alpha_vitkd', 0.00003)
            self.beta_vitkd = getattr(args, 'beta_vitkd', 0.000003)
            self.lambda_vitkd = getattr(args, 'lambda_vitkd', 0.5)
    ...
    def forward(self, inputs, outputs, labels):
        ...
        if self.use_vitkd:
            loss_vitkd = vitkd_solver(
                block_outs_s, 
                block_outs_t, 
                self.vitkd_projector,
                self.alpha_vitkd,
                self.beta_vitkd,
                self.lambda_vitkd
            )
            return loss_base, loss_dist, loss_vitkd,  torch.tensor(0.0, device=loss_base.device), torch.tensor(0.0, device=loss_base.device), torch.tensor(0.0, device=loss_base.device), torch.tensor(0.0, device=loss_base.device), torch.tensor(0.0, device=loss_base.device), torch.tensor(0.0, device=loss_base.device), torch.tensor(0.0, device=loss_base.device), torch.tensor(0.0, device=loss_base.device)
        else:
...
def mf_loss(block_outs_s, block_outs_t, layer_ids_s, layer_ids_t, K, max_patch_num=0, normalize=False, distance='MSE', prototypes=None, projectors_nets=None, KoLeoData=None, KoLeoPrototypes=None, world_size=1):
    ...
    for idx, (id_s, id_t) in enumerate(zip(layer_ids_s, layer_ids_t)):
        ...
        if prototypes[idx].protos[0] is not None:
            ...
            loss_mf_patch, loss_mf_cls, loss_mf_rand, loss_KoLeo_patch_data, loss_KoLeo_cls_data, loss_KoLeo_rand_data, loss_KoLeo_patch_proto, loss_KoLeo_cls_proto, loss_KoLeo_rand_proto = layer_mf_loss(
...
def layer_mf_loss(F_s, F_t, K, normalize=False, distance='MSE', temperature=0.1, eps=1e-8, prototypes=None, projectors_net=None):
    ...
    return loss_mf_patch, loss_mf_cls, loss_mf_rand, torch.tensor(0.0, device=dev), torch.tensor(0.0, device=dev), torch.tensor(0.0, device=dev), torch.tensor(0.0, device=dev), torch.tensor(0.0, device=dev), torch.tensor(0.0, device=dev)
...
def vitkd_solver(
    ...
    low_s = torch.stack([preds_S[0][:,1:,:], preds_S[1][:,1:,:]], dim=1)
    low_t = torch.stack([preds_T[0][:,1:,:], preds_T[1][:,1:,:]], dim=1)
    ...
    '''ViTKD: Mimicking'''
    if vitkd_projector.align2 is not None:
        for i in range(2):
            if i == 0:
                xc = vitkd_projector.align2[i](low_s[:,i]).unsqueeze(1)
            else:
                xc = torch.cat((xc, vitkd_projector.align2[i](low_s[:,i]).unsqueeze(1)),dim=1)
    else:
        xc = low_s
    ...
    '''ViTKD: Generation'''
    if vitkd_projector.align is not None:
        x = vitkd_projector.align(high_s)
    else:
        x = high_s
    ...
    mask_tokens = vitkd_projector.mask_token.repeat(B, N - x.shape[1], 1)
    ...
    x = vitkd_projector.generation(x).flatten(2).transpose(1,2)
